...
```{r}
GUI_juicr("fuwm 3.5 top iso point image.png") #use to choose points
fuwm3.5Tpoints <- read.csv("fuwm 3.5 top iso point image_juicr_extracted_points.csv") %>%
  select(x.coord, y.coord) %>%
  mutate(x.coord = (x.coord*100)/1000, y.coord = (y.coord*100)/670)
```

## Plots
```{r FUWM 3.5 T, fig.width=10.417, fig.height=6.979}
x11(width = 10, height = 6.83)
ggplot(fuwm3.5Tpoints, aes(x = x.coord, y = y.coord)) +
  annotation_custom(rasterGrob(img3.5T, x = unit(0.5, "npc"), y = unit(0.5, "npc")), xmin = 0, xmax = 1, ymin = 0, ymax = 1) +
  xlim(0,1) +
  ylim(0,1) +
